    ...
    const char *mime, int width, int height)
{
    ...
    for (const char *at = mime; (at = strstr(at, "avc1.")) != NULL; at++) {
        ...
        /* Portrait pictures taller than 480 rows (360x640 Shorts) fail
           the first access unit on hardware; 240x426 decodes. */
        if (height > width && height > 480) return false;
...
/* Quality is the picture's short side, so a portrait Short at 360x640
   counts as 360p the same way a 640x360 landscape video does. */
static int youtube_format_quality(const YoutubeFormat *format)
{
                                     ...
                                     int maximum_height)
{
    return format->url[0] != '\0'
        ...
        && youtube_format_quality(format) <= maximum_height
