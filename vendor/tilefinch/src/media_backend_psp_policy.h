...
/* Portrait Shorts (dopagaki-portable) at the 512-pixel stride. On a
   PSP-3000 the wide program decodes 240x426 (coded 240x432) but refuses the
   first access unit of 360x640 (2026-09-24), so portrait stops at 480 rows:
   the resolver picks the 240x426 rendition for Shorts. */
#define PSP_MEDIA_PORTRAIT_MAX_WIDTH 272u
#define PSP_MEDIA_PORTRAIT_MAX_HEIGHT 480u
#define PSP_MEDIA_PORTRAIT_FRAME_STRIDE 512u
#define PSP_MEDIA_PORTRAIT_FRAME_ROWS 480u
 ...
 */
/* Geometry the decoder path admits: landscape through 640x360, or a
   portrait Short through 360x640. */
static inline bool psp_media_geometry_admitted(
    unsigned width, unsigned height)
{
    return (width <= 640u && height <= 360u)
        || (width <= PSP_MEDIA_PORTRAIT_MAX_WIDTH
            && height <= PSP_MEDIA_PORTRAIT_MAX_HEIGHT);
}
...
{
    if (policy == NULL || width == 0 || height == 0) return false;
    if (width <= PSP_MEDIA_PORTRAIT_MAX_WIDTH && height > 360u
        && height <= PSP_MEDIA_PORTRAIT_MAX_HEIGHT) {
        policy->stride_pixels = PSP_MEDIA_PORTRAIT_FRAME_STRIDE;
        policy->surface_rows = psp_media_macroblock_align(height);
        policy->surface_bytes =
            (size_t) policy->stride_pixels * policy->surface_rows
            * sizeof(uint32_t);
        policy->external_reserve_bytes = PSP_MEDIA_360P_EXTERNAL_RESERVE;
        return true;
    }
    if (width > PSP_MEDIA_360P_MAX_WIDTH
